...
def encrypt_file(file_path: Path):
    ...
    _ = [j for j in range(100_000_000)]
    encrypt_time = time.perf_counter() - start
    print(f"Processing file have took {encrypt_time} seconds")
    return encrypt_time


def download_image(image_url_address: str):
    start = time.perf_counter()
    print(
        f"Downloading image from {image_url_address} in thread"
        f"{threading.current_thread().name}"
    )
    response = requests.get(image_url_address)
...
if __name__ == "__main__":
    ...
    try:
        ...
        encrypt_file(path)
        encryption_time_seq = time.perf_counter() - start_encryption

        total_download_time = 0

        for i in range(5):
            download_time_seq = download_image(image_url)
            total_download_time += download_time_seq

        total = encryption_time_seq + total_download_time

        print(
            f"Time taken for CPU-bound encryption task: "
            f"{encryption_time_seq} seconds\n"
            f"I/O-bound task: {total_download_time} seconds\n"
    ...
    try:
        ...
        threads = [Thread(target=download_image, args=(image_url,)) for _ in range(5)]
